...
- **Money** is always an integer number of cents (`Cents`). Use `parseMoney`
  to read user input, `formatMoney` to display amounts with a currency code,
  and `formatCents` to display amounts as a US dollar string (e.g.
  `"$1,234.56"`). Percentages are applied with `percentOf(amount,
  basisPoints)`, where 725 bps = 7.25%.
