...
- **Money** is always an integer number of cents (`Cents`). Use `parseMoney`
  to read user input and `formatMoney` or `formatCents` to display amounts
  (`formatMoney` appends a currency code, `formatCents` renders a `$`-prefixed
  US dollar string). Percentages are applied with
